...
public class AsteroidCameraController : MonoBehaviour
{
	// Zoom to fit params
	...
	void Start()
	{
		...
		SpawnPosition = Path.GetControlPoint(0);
		TeleportPosition = Path.GetControlPoint(Path.ControlPointCount - 1);
		CenterAndZoomToFit();
	}
	...
	private Vector3 CameraTargetPosition()
	{
		...
		if (counter > 0)
		{
			return ClampToField(avg / counter);
		}
		else
		{
			return ClampToField(cam.transform.position);
		}
	}
	...
	{
		float cameraMaxHalfWidth = cam.aspect * MaxSize;
		pos.x = Mathf.Clamp (pos.x, SpawnPosition.x + cameraMaxHalfWidth, TeleportPosition.x - cameraMaxHalfWidth);
		pos.y = Mathf.Clamp (pos.y, SpawnPosition.y + MaxSize, TeleportPosition.y - MaxSize);
		return pos;
	}
